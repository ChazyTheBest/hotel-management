...
const SettingsDropdown = ({ className = '', user, logout }) =>
  <Dropdown
    className={className}
    trigger={user.managesAccountPhotos ?
      <button type="button" className="flex text-sm border-2 border-transparent rounded-full focus:outline-none focus:border-gray-300 transition">
        <img className="h-8 w-8 rounded-full object-cover" src={user.account_photo_url} alt={user.name} />
      </button>
    :
  ...
  >
    ...
    <DropdownLink href={route('user-settings.show')}>
      Account Settings
    </DropdownLink>

    <DropdownLink href={route('booking.index')}>
      My bookings
    </DropdownLink>
