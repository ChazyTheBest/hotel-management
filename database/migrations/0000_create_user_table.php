...
return new class extends Migration
{
    ...
    public function up(): void
    {
        Schema::create(User::getTableName(), function (Blueprint $table) {
            ...
            $table->tinyInteger('role')->default(\App\Enums\Role::USER->value);
            $table->timestamps();
            $table->softDeletes();
        });
        ...
        Schema::create(config('session.table'), function (Blueprint $table) {
            ...
            $table->foreignId(User::getFK())->nullable()->index()->constrained(User::getTableName());
            $table->string('ip_address', 45)->nullable();
            $table->text('user_agent')->nullable();
            $table->longText('payload');
            $table->string('previous_id')->nullable()->index();
